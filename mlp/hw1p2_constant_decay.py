...
def train(epochs, train_dataloader, val_dataloader, weight_init_fn, lr, *args):
    ...
    for epoch in range(epochs):
        ...
        end_epoch = time.time()
        print('Batch Loss:', loss.item(), "Time taken:", end_epoch-start_epoch)
...
def test(model, test_dataloader):
    with torch.no_grad():
        ...
        PrintOutput("/content/drive/My Drive/11-785hw1p2-f19/test_output.csv", total_pred)
...
if __name__ == "__main__":
    loader = WSJ()
    trainX, trainY = loader.dev
    valX, valY = loader.dev
    testX, testY = loader.test

    train_dataset = ASRDataset(trainX, trainY)

    train_loader_args = dict(shuffle=True, batch_size=64, num_workers=8, pin_memory=True) if CUDA\
    ...
    val_loader_args = dict(shuffle=False, batch_size=64, num_workers=8, pin_memory=True) if CUDA\
    ...
    test_loader_args = dict(shuffle=False, batch_size=64, num_workers=8, pin_memory=True) if CUDA\
                    else dict(shuffle=False, batch_size=16)
    ...
    hiddens = [2048, 1024, 1024, 512]
    model = train(epochs, train_dataloader, val_dataloader, init_randn, 1e-3, input_size, OUTPUT_SIZE, hiddens)
