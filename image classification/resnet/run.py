...
def train(model, data_loader, val_loader, test_loader, label_to_org, optimizer, criterion, numEpochs, task='Classification'):
    ...
    for epoch in range(numEpochs):
        ...
        if task == 'Classification':
            ...
            print('########################Train Loss: {:.4f}\tTrain Accuracy: {:.4f}\tVal Loss: {:.4f}\tVal Accuracy: {:.4f}\tTime: {:.4f}\t######################'.
                  format(train_loss, train_acc, val_loss, val_acc, end-start))
...
if __name__ == "__main__":
    ...
    output_channels = {1: 64, 2: 256, 3: 512, 4: 1024, 5: 2048}
    repeats = {2: 3, 3: 4, 4: 6, 5: 3}
    resnet = model.ResNet(3, output_channels, repeats, class_n)
    # resnet.load_state_dict(torch.load('model_5', map_location=torch.device('cpu')))
    resnet.to(device)
    ...
    # optimizer = torch.optim.SGD(resnet.parameters(), lr=learningRate, weight_decay=weightDecay, momentum=0.9)
    optimizer = torch.optim.Adam(resnet.parameters(), lr=learningRate)

    train(resnet, dataloader, valloader, testloader, label_to_org, optimizer, criterion, numEpochs)
